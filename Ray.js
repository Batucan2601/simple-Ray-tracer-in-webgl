...
class Ray{
    ...
    findClosestIntersection( objectList , intersectedObject ){ // that will return a color pixel in the end 
        ...
        if(object.mirror > 0)
        {
            var normal = [intersectionPoint[1][0], intersectionPoint[1][1], intersectionPoint[1][2], 0]
            var r = calculateReflectionVector(this, normal)
    
             //console.log( r )
            newRay = new Ray( intersectionPoint[0] , r , this.lightList) 
            ...
            mirrorColor= newRay.findClosestIntersection( objectList , object )

            totalColor = [object.mirror * mirrorColor[0] + (1 - mirrorColor[0]) * totalColor[0] , object.mirror * mirrorColor[1] + (1 - mirrorColor[1]) * totalColor[1],object.mirror * mirrorColor[2] + (1 - mirrorColor[2]) * totalColor[2]]
            totalColor = mirrorColor
        }
